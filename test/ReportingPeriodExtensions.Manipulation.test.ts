...
describe('ReportingPeriodExtensions.Manipulation --', () => {
  describe('cloneWithAdjustment --', () => {
    ...
    it('should throw Error when granularityOfUnitsToAdd is more granular than component being adjusted', () => {
      // Arrange
      const tests = [{
        reportingPeriod        : new ReportingPeriod(new CalendarMonth(2019, MonthOfYear.July),
          new CalendarMonth(2019, MonthOfYear.July),
        ),
        granularityOfUnitsToAdd: [UnitOfTimeGranularity.Day],
      }, {
        reportingPeriod        : new ReportingPeriod(new CalendarQuarter(2013, QuarterNumber.Q3),
        ...
        reportingPeriod        : new ReportingPeriod(new FiscalMonth(2019, MonthOfYear.July),
          new FiscalMonth(2019, MonthOfYear.July),
        ),
        granularityOfUnitsToAdd: [UnitOfTimeGranularity.Day],
      }, {
        reportingPeriod        : new ReportingPeriod(new FiscalQuarter(2013, QuarterNumber.Q3),
      ...
      tests.forEach(_ => {
        _.granularityOfUnitsToAdd.forEach(granularity => {
          // Assert
          expect(() => _.reportingPeriod.cloneWithAdjustment(ReportingPeriodComponent.Both, 1, granularity))
            .toThrow(/granularityOfUnitsToAdd is more granular than unitOfTime/)
        })
      })
    })
    it('should adjust start of reportingPeriod when ReportingPeriodComponent is Start', () => {
      // Arrange
      const tests = [{
        reportingPeriod        : new ReportingPeriod(new FiscalMonth(2016, MonthNumber.Four),
          new FiscalMonth(2017, MonthNumber.Eleven),
        ),
        unitsToAdd             : 1,
        granularityOfUnitsToAdd: UnitOfTimeGranularity.Month,
        expectedReportingPeriod: new ReportingPeriod(new FiscalMonth(2016, MonthNumber.Five),
          new FiscalMonth(2017, MonthNumber.Eleven),
        ),
      }, {
        reportingPeriod        : new ReportingPeriod(new FiscalMonth(2016, MonthNumber.Four),
        ...
        expectedReportingPeriod: new ReportingPeriod(new FiscalMonth(2015, MonthNumber.Ten),
          new FiscalMonth(2017, MonthNumber.Eleven),
        ),
      }, {
        reportingPeriod        : new ReportingPeriod(new FiscalMonth(2014, MonthNumber.Four),
          new FiscalMonth(2017, MonthNumber.Eleven),
        ),
        unitsToAdd             : 2,
        granularityOfUnitsToAdd: UnitOfTimeGranularity.Year,
        expectedReportingPeriod: new ReportingPeriod(new FiscalMonth(2016, MonthNumber.Four),
          new FiscalMonth(2017, MonthNumber.Eleven),
        ),
      }]
      ...
      for (const test of tests) {
        const actualReportingPeriod = test.reportingPeriod.cloneWithAdjustment(ReportingPeriodComponent.Start,
          test.unitsToAdd,
    ...
    it('should adjust start of reportingPeriod when ReportingPeriodComponent is End', () => {
      ...
      // Act, Assert
      for (const test of tests) {
        const actualReportingPeriod = test.reportingPeriod.cloneWithAdjustment(ReportingPeriodComponent.End,
          test.unitsToAdd,
          test.granularityOfUnitsToAdd,
        )
        expect(actualReportingPeriod.equals(test.expectedReportingPeriod)).toBe(true)
    ...
    it('should adjust start of reportingPeriod when ReportingPeriodComponent is Both', () => {
      ...
      const tests = [{
        ...
        unitsToAdd             : 2,
        granularityOfUnitsToAdd: UnitOfTimeGranularity.Year,
        expectedReportingPeriod: new ReportingPeriod(new FiscalMonth(2018, MonthNumber.Four),
          new FiscalMonth(2019, MonthNumber.Eleven),
        ),
      }]
      ...
      // Act, Assert
      for (const test of tests) {
        const actualReportingPeriod = test.reportingPeriod.cloneWithAdjustment(ReportingPeriodComponent.Both,
          test.unitsToAdd,
          test.granularityOfUnitsToAdd,
        )
        expect(actualReportingPeriod.equals(test.expectedReportingPeriod)).toBe(true)
    ...
    it(
      'should throw Error when adjusting Start and adjusting reportingPeriod causes Start to be greater than End',
      () => {
        // Arrange
        const tests = [{
          reportingPeriod        : new ReportingPeriod(new FiscalMonth(2016, MonthNumber.Four),
  ...
  describe('createPermutations --', () => {
    ...
    it('should return permutations when call for reportingPeriod of CalendarMonth', () => {
      // Arrange
      const reportingPeriod1 = new ReportingPeriod(new CalendarMonth(2016, MonthOfYear.February),
        new CalendarMonth(2016, MonthOfYear.February),
      )
      const reportingPeriod2 = new ReportingPeriod(new CalendarMonth(2016, MonthOfYear.February),
      ...
      expect(permutations2a).toEqual([new ReportingPeriod(new CalendarMonth(2016, MonthOfYear.February),
        new CalendarMonth(2016, MonthOfYear.February),
      ), new ReportingPeriod(
      ...
      expect(permutations2b).toEqual([new ReportingPeriod(new CalendarMonth(2016, MonthOfYear.February),
        new CalendarMonth(2016, MonthOfYear.February),
      ), new ReportingPeriod(
        new CalendarMonth(2016, MonthOfYear.February),
        new CalendarMonth(2016, MonthOfYear.March),
    ...
    it('should return permuations when call for reportingPeriod of FiscalMonth', () => {
      ...
      expect(permutations2a)
        .toEqual([new ReportingPeriod(
          new FiscalMonth(2016, MonthNumber.Two),
          new FiscalMonth(2016, MonthNumber.Two),
        ), new ReportingPeriod(
    ...
    it('should return permuations when call for reportingPeriod of CalendarQuarter', () => {
      // Arrange
      const reportingPeriod1 = new ReportingPeriod(new CalendarQuarter(2016, QuarterNumber.Q2),
        new CalendarQuarter(2016, QuarterNumber.Q2),
      )
      const reportingPeriod2 = new ReportingPeriod(new CalendarQuarter(2016, QuarterNumber.Q2),
      ...
      expect(permutations2a)
        .toEqual([new ReportingPeriod(
          ...
          new CalendarQuarter(2016, QuarterNumber.Q3),
          new CalendarQuarter(2016, QuarterNumber.Q3),
        ), new ReportingPeriod(
          new CalendarQuarter(2016, QuarterNumber.Q4),
          new CalendarQuarter(2016, QuarterNumber.Q4),
        ), new ReportingPeriod(
    ...
    it('should return permuations when call for reportingPeriod of FiscalQuarter', () => {
      // Arrange
      const reportingPeriod1 = new ReportingPeriod(new FiscalQuarter(2016, QuarterNumber.Q2),
        new FiscalQuarter(2016, QuarterNumber.Q2),
      )
      const reportingPeriod2 = new ReportingPeriod(new FiscalQuarter(2016, QuarterNumber.Q2),
      ...
      expect(permutations2a)
        .toEqual([new ReportingPeriod(
          ...
          new FiscalQuarter(2016, QuarterNumber.Q3),
          new FiscalQuarter(2016, QuarterNumber.Q3),
        ), new ReportingPeriod(
          new FiscalQuarter(2016, QuarterNumber.Q4),
          new FiscalQuarter(2016, QuarterNumber.Q4),
        ), new ReportingPeriod(
    ...
    it('should return permuations when call for reportingPeriod of CalendarYear', () => {
      ...
      expect(permutations2a)
        .toEqual([new ReportingPeriod(
          ...
          new CalendarYear(2016),
        ), new ReportingPeriod(new CalendarYear(2017), new CalendarYear(2017)), new ReportingPeriod(new CalendarYear(
          2018), new CalendarYear(2018)), new ReportingPeriod(new CalendarYear(2019), new CalendarYear(2019))])

      expect(permutations2b)
        .toEqual([new ReportingPeriod(
          new CalendarYear(2016),
          new CalendarYear(2016),
        ), new ReportingPeriod(new CalendarYear(2016), new CalendarYear(2017)), new ReportingPeriod(new CalendarYear(
          2016), new CalendarYear(2018)), new ReportingPeriod(
          new CalendarYear(2017),
          new CalendarYear(2017),
        ), new ReportingPeriod(new CalendarYear(2017), new CalendarYear(2018)), new ReportingPeriod(new CalendarYear(
          2017), new CalendarYear(2019)), new ReportingPeriod(
          new CalendarYear(2018),
          new CalendarYear(2018),
        ), new ReportingPeriod(new CalendarYear(2018), new CalendarYear(2019)), new ReportingPeriod(new CalendarYear(
          2019), new CalendarYear(2019))])
    })
    it('should return permuations when call for reportingPeriod of FiscalYear', () => {
      ...
      expect(permutations2a)
        .toEqual([new ReportingPeriod(new FiscalYear(2016), new FiscalYear(2016)), new ReportingPeriod(new FiscalYear(
          2017), new FiscalYear(2017)), new ReportingPeriod(
          new FiscalYear(2018),
          new FiscalYear(2018),
      ...
      expect(permutations2b)
        .toEqual([new ReportingPeriod(new FiscalYear(2016), new FiscalYear(2016)), new ReportingPeriod(new FiscalYear(
          2016), new FiscalYear(2017)), new ReportingPeriod(
          new FiscalYear(2016),
          new FiscalYear(2018),
        ), new ReportingPeriod(new FiscalYear(2017), new FiscalYear(2017)), new ReportingPeriod(
          new FiscalYear(2017),
          new FiscalYear(2018),
        ), new ReportingPeriod(new FiscalYear(2017), new FiscalYear(2019)), new ReportingPeriod(
          new FiscalYear(2018),
          new FiscalYear(2018),
        ), new ReportingPeriod(new FiscalYear(2018), new FiscalYear(2019)), new ReportingPeriod(
          new FiscalYear(2019),
          new FiscalYear(2019),
        )])
    })
  ...
  describe('split --', () => {
    ...
    // tslint:disable-next-line: max-line-length
    it(
      'should return reportingPeriod split into CalendarYear when reportingPeriod is in CalendarYear and granularity is Year',
      () => {
        // Arrange
        ...
        const results = reportingPeriods.map(_ => {
          return {
            actual: _.reportingPeriod.split(UnitOfTimeGranularity.Year),
            expected: _.expectedUnitsOfTime,
          }
        })
    ...
    // tslint:disable-next-line: max-line-length
    it(
      'should return reportingPeriod split into CalendarQuarter when reportingPeriod is in CalendarYear and granularity is Quarter',
      () => {
        // Arrange
        const reportingPeriods = [{
          reportingPeriod    : new ReportingPeriod(new CalendarYear(2017), new CalendarYear(2017)),
          expectedUnitsOfTime: [new CalendarQuarter(2017, QuarterNumber.Q1), new CalendarQuarter(
            2017,
            QuarterNumber.Q2,
          ), new CalendarQuarter(2017, QuarterNumber.Q3), new CalendarQuarter(2017, QuarterNumber.Q4)],
        }, {
          reportingPeriod    : new ReportingPeriod(new CalendarYear(2017), new CalendarYear(2018)),
          expectedUnitsOfTime: [new CalendarQuarter(2017, QuarterNumber.Q1), new CalendarQuarter(
            2017,
            QuarterNumber.Q2,
          ), new CalendarQuarter(2017, QuarterNumber.Q3), new CalendarQuarter(
            2017,
            QuarterNumber.Q4,
          ), new CalendarQuarter(2018, QuarterNumber.Q1), new CalendarQuarter(
            2018,
            QuarterNumber.Q2,
          ), new CalendarQuarter(2018, QuarterNumber.Q3), new CalendarQuarter(2018, QuarterNumber.Q4)],
        }]

        // Act
        const results = reportingPeriods.map(_ => {
          return {
            actual: _.reportingPeriod.split(UnitOfTimeGranularity.Quarter),
            expected: _.expectedUnitsOfTime,
          }
        })
    ...
    // tslint:disable-next-line: max-line-length
    it(
      'should return reportingPeriod split into CalendarMonth when reportingPeriod is in CalendarYear and granularity is Month',
      () => {
        // Arrange
        const reportingPeriods = [{
          reportingPeriod    : new ReportingPeriod(new CalendarYear(2017), new CalendarYear(2017)),
          expectedUnitsOfTime: [new CalendarMonth(2017, MonthOfYear.January), new CalendarMonth(
            2017,
            MonthOfYear.February,
          ), new CalendarMonth(2017, MonthOfYear.March), new CalendarMonth(2017, MonthOfYear.April), new CalendarMonth(
            2017,
            MonthOfYear.May,
          ), new CalendarMonth(2017, MonthOfYear.June), new CalendarMonth(2017, MonthOfYear.July), new CalendarMonth(
            2017,
            MonthOfYear.August,
          ), new CalendarMonth(2017, MonthOfYear.September), new CalendarMonth(
            2017,
            MonthOfYear.October,
          ), new CalendarMonth(2017, MonthOfYear.November), new CalendarMonth(2017, MonthOfYear.December)],
        }, {
          reportingPeriod    : new ReportingPeriod(new CalendarYear(2017), new CalendarYear(2018)),
          expectedUnitsOfTime: [new CalendarMonth(2017, MonthOfYear.January), new CalendarMonth(
            2017,
            ...
            2017,
            MonthOfYear.October,
          ), new CalendarMonth(2017, MonthOfYear.November), new CalendarMonth(
            2017,
            MonthOfYear.December,
          ), new CalendarMonth(2018, MonthOfYear.January), new CalendarMonth(
            2018,
            MonthOfYear.February,
          ), new CalendarMonth(2018, MonthOfYear.March), new CalendarMonth(2018, MonthOfYear.April), new CalendarMonth(
            2018,
            MonthOfYear.May,
          ), new CalendarMonth(2018, MonthOfYear.June), new CalendarMonth(2018, MonthOfYear.July), new CalendarMonth(
            2018,
            MonthOfYear.August,
          ), new CalendarMonth(2018, MonthOfYear.September), new CalendarMonth(
            2018,
            MonthOfYear.October,
          ), new CalendarMonth(2018, MonthOfYear.November), new CalendarMonth(2018, MonthOfYear.December)],
        }]
        // Act
        const results          = reportingPeriods.map(_ => {
          return {
            actual: _.reportingPeriod.split(UnitOfTimeGranularity.Month),
            expected: _.expectedUnitsOfTime,
          }
        })
    ...
    // tslint:disable-next-line: max-line-length
    it(
      'should throw Error when reportingPeriods is in CalendarQuarter and granularity is Year and there is overflow',
      () => {
        // Arrange
        const reportingPeriods = [new ReportingPeriod(
          new CalendarQuarter(2017, QuarterNumber.Q1),
          new CalendarQuarter(2019, QuarterNumber.Q3),
        ), new ReportingPeriod(
          new CalendarQuarter(2017, QuarterNumber.Q2),
          new CalendarQuarter(2019, QuarterNumber.Q4),
        ), new ReportingPeriod(
          new CalendarQuarter(2017, QuarterNumber.Q1),
          new CalendarQuarter(2017, QuarterNumber.Q1),
        ), new ReportingPeriod(
    ...
    // tslint:disable-next-line: max-line-length
    it(
      'should return reportingPeriod split into CalendarYear when reportingPeriod is in CalendarQuarter and granularity is Year',
      () => {
        // Arrange
        const reportingPeriods = [{
          reportingPeriod    : new ReportingPeriod(new CalendarQuarter(2017, QuarterNumber.Q1),
            new CalendarQuarter(2017, QuarterNumber.Q4),
          ),
          expectedUnitsOfTime: [new CalendarYear(2017)],
        }, {
    ...
    // tslint:disable-next-line: max-line-length
    it(
      'should return reportingPeriod split into CalendarQuarter when reportingPeriod is in CalendarQuarter and granularity is Quarter',
      () => {
        // Arrange
        const reportingPeriods = [{
          reportingPeriod    : new ReportingPeriod(new CalendarQuarter(2017, QuarterNumber.Q1),
          ...
          expectedUnitsOfTime: [new CalendarQuarter(2017, QuarterNumber.Q2), new CalendarQuarter(
            2017,
            QuarterNumber.Q3,
          ), new CalendarQuarter(2017, QuarterNumber.Q4), new CalendarQuarter(
            2018,
            QuarterNumber.Q1,
          ), new CalendarQuarter(2018, QuarterNumber.Q2)],
        }]
    ...
    // tslint:disable-next-line: max-line-length
    it(
      'should return reportingPeriod split into CalendarMonth when reportingPeriod is in CalendarQuarter and granularity is Month',
      () => {
        // Arrange
        const reportingPeriods = [{
          reportingPeriod    : new ReportingPeriod(new CalendarQuarter(2017, QuarterNumber.Q1),
          ...
          expectedUnitsOfTime: [new CalendarMonth(2017, MonthOfYear.April), new CalendarMonth(
            2017,
            MonthOfYear.May,
